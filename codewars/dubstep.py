...
def song_decoder(song):
    words = song.split("WUB")
    return ' '.join([i for i in words if len(i) > 0])

# Even better
def song_decoder(song):
    return ' '.join([i for i in song.split("WUB") if len(i) > 0])

# Best
# 1. Replace WUB with whitespace
# 2. Use .split() which splits by whitespace (any amount), leaving only words
def song_decoder(song):
